...
package org.usfirst.frc3244.MechanicalMayham.subsystems;
 ...
 // member variables for Mecanum drive
    ...
    public Drive() {
		...
		for (talonIndex = 0; talonIndex < kMaxNumberOfMotors; talonIndex++) {
			...
			m_talons[talonIndex].configPeakOutputForward(1, Robot.M_WPI_TalonSRX_config_TimeoutMs);
			m_talons[talonIndex].configPeakOutputReverse(-1, Robot.M_WPI_TalonSRX_config_TimeoutMs);
		}
		...
		if (m_useVoltageRamp) {
			for (talonIndex = 0; talonIndex < kMaxNumberOfMotors; talonIndex++) {
				...
				m_talons[talonIndex].configClosedloopRamp(m_closedLoopRamp_sec, Robot.M_WPI_TalonSRX_config_TimeoutMs);
			}
		} else {
			// clear all voltage ramp rates
			for (talonIndex = 0; talonIndex < kMaxNumberOfMotors; talonIndex++) {
				//m_talons[talonIndex].setVoltageRampRate(0.0);
				m_talons[talonIndex].configClosedloopRamp(m_openLoopRamp_sec, Robot.M_WPI_TalonSRX_config_TimeoutMs);
			}
		}
